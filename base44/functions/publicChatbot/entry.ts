...
Deno.serve(async (req) => {
  try {
    ...
    const { proyecto_id, action, message, visitor_id, channel } = await req.json();
    ...
    if (action === 'config') {
      return Response.json({
        ...
        chatbot_config: proyecto.chatbot_config || {},
        ecommerce_config: proyecto.ecommerce_config ? {
          enabled: proyecto.ecommerce_config.enabled,
          platform: proyecto.ecommerce_config.platform,
        } : { enabled: false },
        esquema_color: proyecto.esquema_color,
      });
    }
    ...
    if (action === 'chat') {
      ...
      try {
        ...
        console.log('Error saving user msg:', e.message);
      }

      // ── Load history from ConversacionChat ──
      let history = [];
      try {
      ...
      leadContext += '\nIMPORTANTE: NO pidas datos que ya aparecen arriba.';
...
${config.knowledge_base || config.descripcion || 'Sin información disponible.'}${productContext}`;
    ...
    return Response.json({ error: 'Invalid action. Use "config" or "chat".' }, { status: 400 });
  } catch (error) {
    console.error('publicChatbot error:', error.message);
